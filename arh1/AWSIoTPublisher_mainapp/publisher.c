...
static void my_publisher_aws_callback( wiced_aws_handle_t aws, wiced_aws_event_type_t event, wiced_aws_callback_data_t* data )
{
    ...
    switch ( event )
    {
        ...
        case WICED_AWS_EVENT_UNSUBSCRIBED:
        		WPRINT_APP_INFO(("Unsubscribed\n"));
        	break;

        case WICED_AWS_EVENT_PAYLOAD_RECEIVED:
        		WPRINT_APP_INFO(("Payload Topic=%.*s\nPayload=%.*s\n",(int)data->message.topic_length , data->message.topic
        				,(int)data->message.data_length,data->message.data));
...
void application_start( void )
{
    ...
    wiced_aws_subscribe( aws_connection,"#", WICED_AWS_QOS_ATMOST_ONCE);
    ...
    while ( 1 )
    {
        ...
        if ( do_publish == 1 )
        {
            ...
            if ( ret != WICED_SUCCESS )
            {
                WPRINT_APP_INFO((" Failed fuck you\r\n"));
                break;
            }
            else
            {
